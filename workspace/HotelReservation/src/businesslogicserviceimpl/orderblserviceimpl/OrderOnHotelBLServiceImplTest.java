...
public class OrderOnHotelBLServiceImplTest {

	private OrderOnHotelBLServiceImpl hotelBLServiceImpl;
	...
	@Before
	public void setUp() {
		order1 = new OrderOnHotelVO(new User("txin",100,"18805156300","151250132@smail.nju.edu.cn"),
				OrderState.UNEXECUTED,105,"2016-10-15 12:00","2016-10-16 12:00", "2016-10-15 24:00",
				RoomType.ROOM_STANDARD,1,1,false);
		order2 = new OrderOnHotelVO(new User("txin",100,"18805156300","151250132@smail.nju.edu.cn"),
				OrderState.ABNORMAL,105, "2016-10-16 12:00","2016-10-17 12:00","2016-10-16 24:00",
				RoomType.ROOM_STANDARD,1,1,false);
		order3 = new OrderOnHotelVO(new User("txin",100,"18805156300","151250132@smail.nju.edu.cn"),
				OrderState.CANCELLED,105, "2016-10-17 12:00","2016-10-18 12:00","2016-10-17 24:00",
				RoomType.ROOM_STANDARD,1,1,false);
		order4 = new OrderOnHotelVO(new User("txin",100,"18805156300","151250132@smail.nju.edu.cn"),
				OrderState.EXECUTED,105, "2016-10-18 12:00","2016-10-19 12:00","2016-10-18 24:00",
				RoomType.ROOM_STANDARD,1,1,false);
		hotelBLServiceImpl = new OrderOnHotelBLServiceImpl();
		r1 = new ResultMsg(true, "Ö´ÐÐ³É¹¦£¡");
		hotelList = OrderOnHotelBLServiceImpl.hotelList;
		hotelList.add(order1);
		hotelList.add(order2);
		hotelList.add(order3);
		hotelList.add(order4);
		
		r11 = new OrderOnHotelMsg(order1.getInitiator(), order1.getOrderState(), order1.getPrice(), 
